...
const buildExportResultMock = vi.fn();

vi.mock("../../../core/services/edgepress-archive.ts", () => ({
  buildExportResult: (...args: unknown[]) => buildExportResultMock(...args),
}));
...
describe("export API", () => {
  beforeEach(() => {
    clearTestEnv();
    vi.clearAllMocks();
    buildExportResultMock.mockResolvedValue({
      type: "single",
      data: new Uint8Array([0x1f, 0x8b]),
      filename: "edgepress-export-test.edgepress",
    });
  });
  ...
  it("returns gzip attachment on success", async () => {
    ...
    expect(buildExportResultMock).toHaveBeenCalledTimes(1);
    expect(buildExportResultMock).toHaveBeenCalledWith(
      expect.anything(),
      expect.anything(),
      undefined,
  ...
  it("returns zip attachment when export is split into multiple parts", async () => {
    env.MEDIA_BUCKET = {
      list: vi.fn(),
      get: vi.fn(),
      put: vi.fn(),
    ...
    buildExportResultMock.mockResolvedValue({
      ...
      filename: "edgepress-bundle-test.zip",
      parts: [],
    });

    const { GET } = await import("../export.ts");
    const response = await GET({
      request: new Request("http://localhost/api/export?database=1&media=1"),
      locals: {} as Parameters<typeof GET>[0]["locals"],
    } as Parameters<typeof GET>[0]);

    expect(response.status).toBe(200);
    expect(response.headers.get("Content-Type")).toBe("application/zip");
    expect(response.headers.get("Content-Disposition")).toContain("edgepress-bundle-test.zip");
  });
  ...
  it("returns 500 when buildExport throws", async () => {
    ...
    buildExportResultMock.mockRejectedValue(new Error("Export failed"));
