...
try {
  if (isCI) {
    run(
      "npx wrangler d1 migrations apply edgepress-dev --remote -c wrangler.toml",
      "Migrating remote D1"
    );
    run("npm run db:seed:generate-sql", "Generating seed SQL from seed-data");
    if (existsSync(seedRemoteFile)) {
      run(
        "npx wrangler d1 execute edgepress-dev --remote --file=./drizzle/seed/seed-remote.sql -c wrangler.toml",
        "Seeding remote D1"
      );
    }
  } else {
    run(
      "npx wrangler d1 migrations apply edgepress-dev --local -c wrangler.toml",
      "Migrating local D1"
    );
